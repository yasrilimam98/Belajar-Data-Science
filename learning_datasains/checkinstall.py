...
env_name = "ysrl"
working_folder = "learning_datasains"
env_file = "env_ysrl.yml"
...
def main():
    if "jupyter" in existing_package("base"):
        print("✓ jupyter Notebook berhasil terinstall dengan baik\n")
        if "nb_conda_kernels" in existing_package("base"):
            print("✓ nb_conda_kernels berhasil terinstall dengan baik\n")
            if env_name in existing_env():
                ...
                if req.issubset(exist):
                    print(f"✓ Package berhasil terinstall dengan baik di dalam environment {env_name}\n")
